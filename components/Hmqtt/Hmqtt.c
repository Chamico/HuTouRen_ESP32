...
void TaskHMqtt(void *pvParameters)
{
    ...
    while(false == HwifiGetConnectNetworkOk())
    {
        vTaskDelay(500 / portTICK_PERIOD_MS);
        s_wait_network--;
        if(0 == s_wait_network){
            ESP_LOGI(TAG, "wait network connect timeout");
            ESP_ERROR_CHECK(ESP_FAIL);
        }
    }
    ESP_LOGI(TAG, "wait network connect success");

    mqtt_app_start();

    while (1) {
        
        printf("Running task Hmqtt\n");
